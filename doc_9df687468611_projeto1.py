...
def criar_tabelas(conn):
    ...
    CREATE TABLE IF NOT EXISTS preparacoes (
        ...
        forma_preparo_id INTEGER REFERENCES formas_preparo(id),
    ...
    # Tabela: Prato-Preparações (relacionamento)
    cursor.execute("""
...
def inserir_ingredientes(conn):
    ...
    ingredientes = [
        ...
        # LEGUMINOSAS
        ('Feijão carioca (cru)', 'leguminosas e derivados', 'g', 337, 60, 20, 1.5, 15, 140, 8.2, 1200, 18, 0, 5.8, 0),
        ('Feijão preto (cru)', 'leguminosas e derivados', 'g', 328, 58, 21, 1.3, 16, 130, 9.5, 1150, 20, 0, 5.2, 0),
        ('Lentilha (crua)', 'leguminosas e derivados', 'g', 353, 60, 25, 1.1, 11, 50, 7.6, 650, 15, 0, 4.8, 0),
        ('Grão de bico (cru)', 'leguminosas e derivados', 'g', 364, 61, 19, 6, 17, 105, 6.2, 875, 12, 0, 11, 0),
        
        # CEREAIS
        ('Arroz branco (cru)', 'cereais e derivados', 'g', 365, 78, 7, 0.7, 1.3, 28, 2.1, 350, 8, 0, 7.8, 0),
        ('Arroz integral (cru)', 'cereais e derivados', 'g', 370, 77, 8, 2.9, 3.5, 33, 2.5, 420, 10, 0, 9.5, 0),
        ('Macarrão (cru)', 'cereais e derivados', 'g', 371, 75, 13, 1.5, 3.2, 30, 3.5, 400, 12, 0, 8.2, 0),
        ('Pão francês', 'cereais e derivados', 'g', 293, 57, 8, 3.5, 2.5, 150, 3.2, 580, 5, 0, 12, 0),
        ('Aveia em flocos', 'cereais e derivados', 'g', 389, 66, 17, 7, 10, 54, 4.7, 410, 10, 0, 11.5, 0),
        ...
        # GORDURAS E ÓLEOS
        ('Azeite de oliva', 'aromatizantes, gorduras e óleos', 'ml', 884, 0, 0, 100, 0, 1, 0, 2, 0, 0, 0, 14, 0),
        ('Óleo de soja', 'aromatizantes, gorduras e óleos', 'ml', 884, 0, 0, 100, 0, 0, 0, 2, 0, 0, 0, 16, 0),
...
def inserir_preparacoes(conn):
    ...
    cursor.execute("SELECT id, nome, tipo_alimento FROM ingredientes")
...
def main():
    ...
    try:
        ...
        print("\n[2/6] Inserindo formas de preparo...")
        inserir_formas_preparo(conn)
        
        # Inserir ingredientes
        print("\n[3/6] Inserindo ingredientes...")
        inserir_ingredientes(conn)
        
        # Inserir preparações
        print("\n[4/6] Inserindo preparações...")
        inserir_preparacoes(conn)
        
        # Inserir tipos de prato
        print("\n[5/6] Inserindo tipos de prato...")
        inserir_tipos_prato(conn)
        
        # Inserir pratos
        print("\n[6/6] Inserindo pratos...")
        inserir_pratos(conn)
